...
12. 妆容场景分类：新增7种场景分类（日常、通勤、上课、约会、聚会、面试、旅行），默认选中旅行，采用3列网格布局，点击切换选中状态（粉色边框和背景高亮）
13. 灵感详情页返回箭头：顶部添加返回箭头按钮，点击直接返回灵感首页，无需确认弹窗
14. 分享报告按钮样式统一：与保存至相册按钮样式一致（白色背景、边框、深棕色文字）
15. 查看效果图直接生成：报告页查看效果图按钮显示所需积分（如15积分），点击后跳转姿造美学功能区并自动生成效果图，无需再次点击生成按钮；直接点击姿造美学功能区仍需手动点击生成效果图
16. 面部分析去重：面部形体分析标签页保存至相册按钮下方不重复显示其他标签页内容，妆容、穿搭、发型、美容美体推荐内容仅在各自标签页中展示
17. 文字语言统一：所有界面文字转换为简体中文
...
- 生活计划管理
- 每日打卡功能
- 妆容子模块（效果图选择、化妆键、我的课程、颜色调整、场景分类选择）
...
- 灵感详情页（顶部返回箭头按钮、风格标签、适用体型/脸型、方案描述、适合原因）
- 收藏和评论功能
